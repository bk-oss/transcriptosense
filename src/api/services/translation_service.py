...
def _translate_chunk(chunk: str, target: str, src: str) -> tuple[str, Optional[str], list[str]]:
    ...
    # 1) Groq (primary)
    try:
        translated = _groq_translate(chunk, target)
        if translated:
            engine_used = "groq"
    ...
    if translated is None:
        try:
            ...
            try:
                result = argostranslate.translate.translate(chunk, from_code, target)
                if result:
                    translated = result
                    engine_used = "argos"
    ...
    if translated is None:
        try:
            translated = _google_translate_with_timeout(chunk, src, target)
            engine_used = "google"
