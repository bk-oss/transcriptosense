...
from src.api.services.diarization_service import diarize_audio
...
async def transcribe(
    ...
    try:
        ...
        # Run preprocessing to generate 16k WAV + VAD JSON so diarization can fall back to VAD
        try:
            from src.ingestion.preprocess import preprocess_audio
            work_dir = Path(_BASE_DIR) / "data"
            preprocess_audio(Path(temp_path), work_dir=work_dir)
        except Exception:
            # Preprocess failed — continue, diarization will fallback if possible
            pass

        result = transcribe_audio_file(temp_path, language=lang_hint)

        # Preserve Deepgram speaker diarization if available, otherwise run pyannote/VAD.
        if not result.get("speakers"):
            vad_json = os.path.join(_BASE_DIR, "data", "interim", f"{Path(temp_path).stem}_vad.json")
            diar = diarize_audio(temp_path, vad_json_path=vad_json)
            if diar.get("speakers"):
                result["speakers"] = json.dumps(diar.get("speakers"))
                result["has_diarization"] = bool(diar.get("has_diarization", False))
        ...
        return TranscriptionResponse(
            ...
            filename=file.filename,
            language=result.get("language", "unknown"),
            transcription=result.get("diarized_text", result.get("text", "")),
            original_text=result.get("plain_text", result.get("text", "")),
            diarized_text=result.get("diarized_text", result.get("text", "")),
